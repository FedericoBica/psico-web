...
// src/actions/product/create-update-product.ts
...
const productSchema = z.object({
  ...
  // Físico
  hasPhysical:   z.preprocess(
    (val) => val === 'true' || val === true || val === 'on',
    z.boolean()
  ).default(false),
  physicalPrice: z.coerce.number().min(0).optional().nullable().transform((val) => val ? Number(val.toFixed(2)) : null),
});
...
export const createUpdateProduct = async (formData: FormData) => {
  ...
  try {
    const prismaTx = await prisma.$transaction(async (tx: TransactionClient) => {
      const tagsArray = rest.tags.split(',').map((tag: string) => tag.trim().toLowerCase());

      const productData = { ...rest, tags: tagsArray };
      ...
      if (imagesFiles.length > 0 && imagesFiles[0].size > 0) {
        ...
        await tx.productImage.createMany({
          data: images
            ...
            .map((url: string) => ({ url, productId: savedProduct.id })),
        });
      }
    ...
    const uploadPromises = images.map(async (image: File): Promise<string | null> => {
      try {
        ...
        return cloudinary.uploader
          .upload(`data:image/${ext};base64,${base64Image}`, { folder: 'psico-web-ebooks' })
          .then((r) => r.secure_url);
      } catch { return null; }
    });
